...
% Title (fill manually)
% ------------------------------------------------
\title{Lista 4 --- Teoria Microeconômica (PPGE-UFF)}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{images/Pasted image 20260416180100.png}
  \caption{Fonte: \textcite{Cowell2004}, p.~191. Na figura, os possíveis
...
\parencite[, p. 191]{Cowell2004}.\footnote{Note-se que esta definição
  \textbf{não} é a mesma que \textcite{Mas-Colell1995} fazem: MWG definem
...
A riqueza deste indivíduo, em ambos os cenários, será
...
\[
\begin{cases}
\text{Sem auditoria: } y_n \coloneqq y - tx \\
\text{Com auditoria: } y_a \coloneqq (1-(1+s)t)y + stx 
\end{cases}
\]
...
Através da condição de primeira ordem, mostre que, quando \(s=\frac{1-p}{p}\), \(x=y\), e que, quando \(s>\frac{(1-p)}{p}\), valerá que \(x < y\).

\begin{remark}  
O motivo da fórmula da renda disponível pós-auditoria ficar daquela forma é o seguinte.
Supondo que \textit{vá haver auditoria}, temos os casos extremos 1) para
\(x=y\), naturalmente ele terá \((1-t)y\) de renda disponível em ambos
os casos (afinal, quem não deve, não teme); 2) com \(x=0\), ele fica com
\((1 - (1+s)t)y\): ele paga o imposto que deveria, i.e. $ty$, mais uma multa $sty$, sendo sua renda disponível $y$ menos estes dedutíveis. 

Estes dois casos compõem o máximo e o mínimo, respectivamente ao declarar $x=y$ e $x=0$, que o indivíduo conseguiria reter como renda disponível \textit{sob auditoria}. Chamando de $Y(x)$ a renda disponível que ele reterá ao declarar $x$, então, por triângulos equivalentes --- ``este $\Delta Y(x)$ para este $\Delta x$ $=$ aquele $\Delta f(x)$ para aquele $\Delta x$'', assumindo uma $Y(x)$ linear em $x$ ---, temos que

\begin{align*}
\frac{[(1-t)y] - [(1-(1+s)t)y]}{y - 0} = \frac{Y(x) - [(1-t)y]}{x - 0} \\
\therefore Y(x) = (1-(1+s)t)y + stx
\end{align*}
\end{remark}
\end{exercise}
...
\textit{$p$-norma ponderada}\footnote{\url{https://en.wikipedia.org/wiki/Lp_space\#The_p-norm_in_finite_dimensions}.} (na verdade, uma \((1-\rho)\)-norma ponderada), i.e.
\[
...
\(w-L\)). Suponha que estes indivíduos reservem uma quantidade \(q\) de suas
rendas (ficando com renda \(w-q\)) em um ``\textit{pool}'' monetário comum, de tal
forma que indivíduos que sofram esta perda $L$ possam ressarcir-se através
deste \textit{pool}.
...
utilidade for \(u(x) = ax\) (\(a>0\)), então ele estaria disposto a pagar este mesmo valor. (Dica: a fórmula do equivalente certo diz sobre \textit{a renda equivalente} que dá a utilidade média ao indivíduo, enquanto aqui queremos saber \textit{o valor subtraído da renda} que ele aceita para se desfazer do risco, i.e. o quanto ele se dispõe \textit{a pagar}.)
...
caso \(\langle r \rangle>0\)\footnote{Por ser avesso ao risco. Não tende
  a ser o mesmo para amante ao risco.}. Portanto, tomando também que
