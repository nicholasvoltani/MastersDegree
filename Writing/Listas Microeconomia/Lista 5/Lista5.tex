...
o será em maior escala (\textit{ceteris paribus}, em particular as
proporções de insumos e produtos etc.).
  \item Retornos
...
\begin{definition}
Uma função \(f: \mathbb{R}^{n} \to \mathbb{R}_{+}\) é dita ser uma
...
\(Y \in \mathbb{R}^{n+1}\) e função produção
...
Supondo que a firma tenha uma função produção
\(f(x_{1},\dots,x_{n}) \equiv f(x)\), o problema se escreve\footnote{Pois,
  lembrando da definição de função produção, \(f(z)\) é a quantidade
...
(Note que, como não existe almoço grátis na vida, este caso não é
sustentável na realidade. O que \textit{pode} acontecer é que um certo
processo de produção tenha \textit{localmente} retornos de escala
não-decrescentes, mas que, a escalas de produção maiores, ele tenha
\textit{rendimentos marginais decrescentes}.)
\end{exercise}

\begin{exercise}
Suponha que \(f: \mathbb{R} \to \mathbb{R}^{L-1}\) seja a função de
...
e respectivas funções custo:\footnote{Citando o honorável
...
Tanto a firma quanto o consumidor são assumidos como tomadores de
preços: \(x\) tem preço \(p\), e \(m\) tem preço \(1\)\footnote{I.e.
...
capital, proporção esta ditada pela função produção \(f\).
