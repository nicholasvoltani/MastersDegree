...
\begin{exercise}
  ...
  Através da função utilidade de Cobb-Douglas \(U(x_{1},x_{2}) = x_{1}^{\alpha}x_{2}^{1-\alpha}\), e supondo vetor de preços \(\vec{p} = (p_1,)\) e restrição orçamentária \(w\), demonstre que vale a equação de Slutsky. Para isso, resolva os seguintes passos: 
  
  \begin{enumerate}
  \item Obtenha as demandas walrasianas \(x^*(\vec{p}, w)\) e \(y^*(\vec{p},w)\) pelo problema de maximização de utilidade 
  \item Com as demandas walrasianas, obtenha a função de utilidade indireta \(v(\vec{p}, w) = U(x^*(\vec{p},w), y^*(\vec{p},w))\), a ser denotada como \(\bar{u} \in \mathbb{R}\)
  \item Obtenha as demandas hicksianas (também chamadas de demandas \textit{compensadas} de Hicks) \(h_{x}(\vec{p}, \bar{u})\) e \(h_{y}(\vec{p}, \bar{u})\). Para isso, utilize a identidade de Roy para cada bem \(i\):
  \[
...
\item A equação de Slutsky tem a forma \(\frac{ \partial x^*_{i} }{ \partial p_{i} } = \frac{ \partial h_{i} }{ \partial p_{i} } - x^*_{i} \frac{ \partial x_{i}^* }{ \partial w }\) para cada bem \(i\). Calcule cada um dos termos, e confirme que a equação é válida.
\end{enumerate}
...
b) Dadas probabilidades \(P(E \mid I)\) e \(P(E \mid \lnot I)\), junto de \(P(I)\), pode-se inferir todas as probabilidades dos eventos \(A, B, C, D\) pela definição de probabilidades bayesianas:
\[
\begin{cases}
