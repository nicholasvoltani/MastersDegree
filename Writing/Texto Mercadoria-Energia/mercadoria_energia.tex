...
\usepackage{bookmark}
\addbibresource{mercadoria_energia.bib}
...
Há valor mesmo na misteriosa eletricidade que chega nos lares das máquinas industriais (e também nos lares de seus leigos empregados), valor este devido não só à sua produção original --- p. ex. na extração de carvão ou petróleo ---, como também pela depreciação envolvida em sua transmissão e conversão em energia elétrica (turbinas, cabos elétricos, transformadores, baterias, etc), além de processos auxiliares, como monitoramento do funcionamento adequado das redes elétricas, etc. A questão do valor da eletricidade necessariamente requer a consideração da sua cadeia de valor, desde sua geração original (energia primária), passando por sua transmissão e distribuição, até sua venda ao consumidor final \cite{Richter2012, PintoJr2007}.
...
Contudo, como este ``refugo energético'' faz parte de um dado processo de produção de energia útil, sendo indissociável deste, então mesmo as mercadorias ``desperdiçadas'' neste processo são ``contabilizadas'' no valor da energia útil final. (Novamente: a energia em si não possui valor, e sim as mercadorias cujo usufruto provêm energia útil. Falar do ``valor da energia'' é meramente uma abreviação desta assertiva.) A história é análoga à geração de \textit{devil's dust} apontado por Marx \cite[p. 282]{Marx2017}: caso 15 libras de algodão \textit{necessariamente} tenham de ser desperdiçadas para transformar 115 libras de algodão em fios, então o valor destas 15 libras de algodão também comporão o valor final dos fios. O material que é necessariamente desperdiçado em algum processo produtivo também transfere seu valor ao produto final, porquanto a produção deste produto requer, necessariamente, que uma parcela seja jogada fora; ou seja, este produto \textit{não pode ser produzido} se não houver este refugo, ele é um \textit{sine qua non} de sua produção e, portanto, compõe parte de seu valor, da mesma forma como se fizesse parte do corpo físico deste produto final. 

No que tange o processo econômico, a energia, tal qual as ideias de um empreendedor ávido por louros e louvores, não vale nada se não for \textit{efetivada} em algo objetivo.\footnote{Similar à força de trabalho/``capacidade de trabalho'': ``Se [a capacidade de trabalho] não é vendida, ela não serve de nada para o trabalhador (...). Ele descobre, então, com Sismondi: ``A capacidade de trabalho [...] não é nada quando não é vendida''.'' \cite[p. 248]{Marx2017}} Uma coisa só tem o potencial de fazer algo quando se espera que ela efetivamente cumpra este potencial, ou seja, que sua \textit{potentia} torne-se \textit{actu}; caso contrário, seu potencial não passa de \textit{wishful thinking}. Ou seja, a energia química potencial do carvão \textit{per se} não importa, e sim a liberação dessa energia em fornos de combustão que sirvam para algo; não importa a tensão alternada da corrente elétrica que chega numa fábrica, e sim como ela traz à vida as máquinas mortas que ``empunham'' o trabalho vivo.
