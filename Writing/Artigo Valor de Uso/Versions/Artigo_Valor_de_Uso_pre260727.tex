...
De fato, é verdade que o ser \textit{natural} de um objeto tem precedência ontológica quanto a seu ser \textit{social}, qual seja, sua potencialidade de satisfazer necessidades humanas. Afinal, um objeto é social necessariamente porque é natural, mas a recíproca não é sempre verdadeira, pois existem objetos naturais que não têm qualidade social, e.g. objetos que não são úteis ao homem. Portanto, é condição necessária, em particular, de todo valor de uso que ele seja um objeto natural --- dito de outra forma, é uma \textit{prioridade ontológica} ---, mas isso não é condição suficiente para que ele seja, de fato, \textit{útil}. 
...
\section{O trabalho e o pôr teleológico}
