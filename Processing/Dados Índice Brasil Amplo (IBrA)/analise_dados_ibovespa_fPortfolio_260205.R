...
log_retornos_mercado_estimacao <- ibra2 %>% 
  arrange(Data) %>% 
  select(Data, all_of(todas_acoes)
  ) %>% 
  mutate(across(all_of(todas_acoes), 
                ~ log(.x / dplyr::lag(.x))
  )
  ) %>%
  slice(2:n()) %>% 
  dplyr::filter(Data >= DATA_INICIAL_REFERENCIA &
                  Data < DATA_FINAL_REFERENCIA) 

log_retornos_totais_estimacao <- log_retornos_mercado_estimacao %>% 
  summarise(across(all_of(todas_acoes),
                   sum)
  ) %>% 
  pivot_longer(all_of(todas_acoes),
               names_to = "tickets",
               values_to = "soma_log_retornos"
  ) %>%
  arrange(soma_log_retornos)

melhores_acoes_mercado_estimacao <- log_retornos_totais_estimacao %>% 
  dplyr::filter(soma_log_retornos > 1) %>% 
  select(tickets) %>%  
  unique

log_retornos_totais_estimacao <- log_retornos_totais_estimacao %>% 
  left_join(composicao_ibra %>% 
              select(Nome,
                     Código,
                     `Setor Econômico|Bovespa`,
                     `Comp carteira|Mais Recente|em %|Ibrasil`),
            join_by(tickets == "Código"))


# Visualização de matriz de correlação entre log-RETORNOS das ações
corrplot(log_retornos_mercado_estimacao %>% 
           select(all_of(melhores_acoes_mercado_estimacao$tickets)) %>% 
...
acoes_escolhidas <- melhores_acoes_mercado_estimacao$tickets
...
to_plot <- data.frame(
  ...
  risky_plus_free = log_returns_risk_plus_free_ptf %>% cumsum %>% exp %>% `-`(1),
  risky_short = log_returns_risky_ptf_short %>% cumsum %>% exp %>% `-`(1),
  risky_plus_free_short = log_returns_risk_plus_free_ptf_short %>% cumsum %>% exp %>% `-`(1),
...
ggplot(to_plot_total, aes(x = ano_mes, 
  ...
  scale_color_manual(
    ...
    labels = c(paste0("Ibovespa (",
                      round(sum(log_retornos_compra_venda_mercado$IBOV) %>% exp %>% `-`(1), 2), ")"),
               ...
               paste0("Risky+Short (", round(corr_risky_short_ibov, 2), ", ", 
                      round(sum(log_returns_risky_ptf_short) %>% exp %>% `-`(1), 2), ")"),
               paste0("Risky+Free (", round(corr_risk_plus_free_ibov, 2), ", ", 
                      round(sum(log_returns_risk_plus_free_ptf) %>% exp %>% `-`(1), 2), ")"),
               paste0("Risky+Free+Short (", round(corr_risk_plus_free_short_ibov, 2), ", ", 
                      round(sum(log_returns_risk_plus_free_ptf_short) %>% exp %>% `-`(1), 2), ")")
               )
    ) +
  theme_minimal()
